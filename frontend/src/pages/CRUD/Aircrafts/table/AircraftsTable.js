...
import * as dataFormat from 'pages/CRUD/Aircrafts/table/AircraftsDataFormatters';

import actions from 'actions/aircrafts/aircraftsListActions';
...
const AircraftsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Registration', title: 'registration' },
  ...
  const count = useSelector((store) => store.aircrafts.list.count);
  const modalOpen = useSelector((store) => store.aircrafts.list.modalOpen);
  const rows = useSelector((store) => store.aircrafts.list.rows);
  const idToDelete = useSelector((store) => store.aircrafts.list.idToDelete);
  ...
  const getAircraftsCSV = async () => {
    const response = await axios({
      url: '/aircrafts?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'aircraftsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Aircrafts')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/aircrafts/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/aircrafts/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Aircrafts'
                : 'http://localhost:8080/api-docs/#/Aircrafts'
            }
          >
            API documentation for aircrafts
          </LinkMaterial>
        </div>
      </Widget>
