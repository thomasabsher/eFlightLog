...
import * as dataFormat from 'pages/CRUD/Pilots/table/PilotsDataFormatters';

import actions from 'actions/pilots/pilotsListActions';
...
const PilotsTable = () => {
  ...
  const count = useSelector((store) => store.pilots.list.count);
  const modalOpen = useSelector((store) => store.pilots.list.modalOpen);
  const rows = useSelector((store) => store.pilots.list.rows);
  const idToDelete = useSelector((store) => store.pilots.list.idToDelete);
  ...
  const getPilotsCSV = async () => {
    const response = await axios({
      url: '/pilots?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Pilots')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/pilots/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/pilots/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Pilots'
                : 'http://localhost:8080/api-docs/#/Pilots'
            }
          >
            API documentation for pilots
          </LinkMaterial>
        </div>
      </Widget>
